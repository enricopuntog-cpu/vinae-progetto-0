...
# 12e/12f/12g e l'automazione stessa. Il resto produce uno skip dichiarato.
...
if [ "$relevant" = "false" ]; then
  echo "::notice title=Gate 12g saltato::Nessuna migrazione, griglia 12e/12f/12g o configurazione Supabase modificata."
  if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
    printf '%s\n' "### Gate 12g — SKIP intenzionale" "" \
      "Nessun percorso pertinente modificato (migrazioni, config Supabase, griglie 12e/12f/12g, workflow)." \
      >> "$GITHUB_STEP_SUMMARY"
  fi
fi
