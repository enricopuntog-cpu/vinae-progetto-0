...
# 12e/12f/12g/12h/12i e l'automazione stessa. Il resto produce uno skip dichiarato.
...
is_relevant_path() {
  case "$1" in
    supabase/migrations/* | supabase/config.toml | supabase/seed.sql \
      ...
      | supabase/tests/12h_* | supabase/tests/12i_* \
...
if [ "$relevant" = "false" ]; then
  echo "::notice title=Gate 12g saltato::Nessuna migrazione, griglia 12e/12f/12g/12h/12i o configurazione Supabase modificata."
  if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
    printf '%s\n' "### Gate 12g — SKIP intenzionale" "" \
      "Nessun percorso pertinente modificato (migrazioni, config Supabase, griglie 12e/12f/12g/12h/12i, workflow)." \
      >> "$GITHUB_STEP_SUMMARY"
  fi
fi
