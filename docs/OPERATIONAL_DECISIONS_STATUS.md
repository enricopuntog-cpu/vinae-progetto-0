...
- `main` protetto dal ruleset `main-protection` e da `.github/CODEOWNERS`;
  ...
  `main` e dichiarati dai workflow con secret;
...
| Accessi del delegato | Matrice pronta, nessun accesso concesso | `docs/EMERGENCY_DELEGATE_ACCESS_MATRIX.md`. Ruleset su `main` e `CODEOWNERS` attivi, environment limitati a `main` collegati ai workflow (PR #152). Prima di invitare la persona su GitHub: spostare i valori dei sei secret negli environment (autorizzazione di Enrico; oggi un workflow su un branch li legge ancora); scelta del modello di disaster recovery A (operativo limitato) o B (break-glass completo, che include la decisione sulla chiave `age`); accettazione del rischio *Developer* Supabase (piano Pro, niente *Read-only*). Allarme backup pronto: basta la variabile `BACKUP_ALERT_EXTRA_MENTIONS`. |
