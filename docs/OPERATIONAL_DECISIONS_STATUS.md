...
- MFA `aal2` imposta dal database per `emergency_delegate` in
  `incident_notice_set` e pagina `/account/sicurezza` (TOTP Supabase) per i
  ruoli di continuità; griglia 12h a 21 controlli e prova REST MFA nel gate CI;
- `main` protetto dal ruleset `main-protection` e da `.github/CODEOWNERS`;
  environment GitHub `production-backup` e `production-payouts` limitati a
  `main` e dichiarati dai workflow con secret, che contengono gli unici valori
  dei sei secret (zero secret di repository; prova negativa da branch, anche
  con `deployment: false`, rifiutata);
- modello di disaster recovery della Beta: **A — delegato operativo limitato**
  (restore catastrofico a Enrico; B da rivalutare prima dei pagamenti reali);
...
| Delegato di emergenza | **PREPARATO / persona non ancora nominata** | Preparazione tecnica e operativa chiusa il 23/09/2026: ruolo `emergency_delegate` verificato dalla griglia 12h (21/21 con i controlli MFA, anche nel gate CI) e sul catalogo di produzione, senza privilegi extra; banner consentito al delegato solo in `aal2` (MFA TOTP, PR #152); mandato, onboarding, drill trimestrale e revoca in `docs/EMERGENCY_DELEGATE_ONBOARDING.md`; procedura breve e checklist di riapertura in `docs/EMERGENCY_DELEGATE_INCIDENT_CARD.md`. Modello di disaster recovery **A** (operativo limitato) scelto da Enrico il 24/09/2026. Manca solo la nomina della persona e l'onboarding reale con MFA e accessi provati. Non è chiusa finché questo non avviene. |
| Accessi del delegato | Matrice pronta, nessun accesso concesso | `docs/EMERGENCY_DELEGATE_ACCESS_MATRIX.md`. Ruleset su `main` e `CODEOWNERS` attivi, environment limitati a `main` collegati ai workflow (PR #152). Secret spostati negli environment il 23–24/09/2026 (zero a livello di repository; backup, freshness watch e payout verificati dopo lo spostamento; un workflow su un altro branch non li riceve, anche con `deployment: false`). Modello A scelto: niente chiave `age` né ruoli Supabase *Owner*/*Administrator* al delegato. Resta da accettare il rischio *Developer* Supabase (piano Pro, niente *Read-only*) alla nomina. Allarme backup pronto: basta la variabile `BACKUP_ALERT_EXTRA_MENTIONS`. |
