...
export const Route = createFileRoute("/community/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: t },
        { name: "description", content: c?.descrizione ?? "Club Vinea Wine Club" },
        { property: "og:title", content: t },
        { property: "og:description", content: c?.descrizione ?? "Club Vinea Wine Club" },
        ...(c
          ? [
  ...
  notFoundComponent: () => (
    <div className="rounded-2xl border border-border bg-card p-10 text-center">
      ...
      <Button asChild className="mt-4 bg-bordeaux hover:bg-bordeaux/90">
        <Link to="/community">Torna ai club</Link>
      </Button>
    </div>
  ),
        ...
        <div className="relative flex flex-col gap-4 p-6 text-crema md:p-10">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              className="border-crema/40 bg-transparent text-crema hover:bg-crema/10"
            >
              <Link to="/community">Scopri altri club</Link>
            </Button>
          </div>
        </div>
      </section>
